<template>
  <div id="list">
    <ul v-if="currentList=='Enter Group Chat'">
      <!--群聊列表-->
      <p style="padding: 2px 4px;height: 20px">Group Chat List</p>
    ...
    <!--机器人-->
    <ul v-if="currentList=='Chat with Bot'">
      <p style="padding: 2px 4px;height: 20px">Come and chat with the robot quickly！</p>
      <li :class="{ active: currentSession?'机器人'== currentSession.username:false }"
        ...
        <p class="name">Wali (intelligent reply)</p>
      </li>
    </ul>
    <!--用户列表-->
    <el-scrollbar      wrap-class="userList" wrap-style="height:600px;"
                       view-style="height:100%;" :native="false">
      <ul v-if="currentList=='Private Chat'" >
        <p style="padding: 2px 4px;height: 20px" @click="add">Users List</p>
