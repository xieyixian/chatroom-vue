        ...
        if ("POST".equals(request.getMethod()) && "/doLogin".equals(request.getServletPath())) {
            ...
            try {
                ...
                if (!code.toLowerCase().equals(verify_code.toLowerCase())) {
                    //输出json
                    writer.write(new ObjectMapper().writeValueAsString(RespBean.error("The verification code is incorrect!")));
                    return;
                } else {
        ...
        //管理端登录请求
            ...
            try {
                if (!mailCode.equals(verifyCode)) {
                    printWriter.write(new ObjectMapper().writeValueAsString(RespBean.error("The verification code is incorrect!")));
                } else {
                    filterChain.doFilter(request, response);
                }
            } catch (NullPointerException e) {
